...
func TestSigningMethodEd25519_Alg(t *testing.T) {
	if crypto.SigningMethodEd25519i.Alg() != "Ed25519" {
		t.Fatal("wrong alg")
	}
}

func TestSigningMethodEd25519_Sign(t *testing.T) {
	sk, err := photo.UnmarshalPrivateKeyFromString(privateKey)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSigningMethodEd25519_Verify(t *testing.T) {
	pk, err := photo.UnmarshalPublicKeyFromString(publicKey)
	if err != nil {
		t.Fatal(err)
	}
	for _, data := range ed25519TestData {
...
func TestGenerateEd25519Token(t *testing.T) {
	...
	_, err = jwt.NewWithClaims(crypto.SigningMethodEd25519i, claims).SignedString(sk)
	if err != nil {
		t.Fatal(err)
	}
}
