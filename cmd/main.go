...
import (
	...
	coreCmds "gx/ipfs/QmatUACvrFK3xYg1nd2iLAKfz7Yy5YB56tnzBYHpqiUuhn/go-ipfs/core/commands"
	"gx/ipfs/QmatUACvrFK3xYg1nd2iLAKfz7Yy5YB56tnzBYHpqiUuhn/go-ipfs/core/corehttp"
	"gx/ipfs/QmatUACvrFK3xYg1nd2iLAKfz7Yy5YB56tnzBYHpqiUuhn/go-ipfs/plugin/loader"
...
var log = logging.Logger("cmd/textile")
...
// command ought to be executed on a textile daemon.
//
...
func commandShouldRunOnDaemon(details cmdDetails, req *cmds.Request, root *cmds.Command, cctx *oldcmds.Context) (http.Client, error) {
	...
	if err == repo.ErrApiNotRunning {
		if apiAddrStr != "" && req.Command != startCmd {
	...
	if client != nil {
		if details.cannotRunOnDaemon {
			...
			if daemonLocked, _ := fsrepo.LockedByOtherProcess(cctx.ConfigRoot); daemonLocked {
				return nil, cmds.ClientError("textile daemon is running. please stop it to run this command")
			}
			return nil, nil
		}
	...
	if details.cannotRunOnClient {
		return nil, cmds.ClientError("must run on the textile daemon")
	}
...
var apiFileErrorFmt string = `Failed to parse '%[1]s/api' file.
	error: %[2]s
If you're sure textile-go isn't running, you can just delete it.
`
var checkIPFSUnixFmt = "Otherwise check:\n\tps aux | grep textile"
var checkIPFSWinFmt = "Otherwise check:\n\ttasklist | findstr textile"
