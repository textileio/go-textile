...
func initDatabaseTables(db *sql.DB, pin string) error {
    ...
    create table files (mill text not null, checksum text not null, hash text not null, key text not null, media text not null, size integer not null, added integer not null, meta blob, primary key (mill, checksum));
