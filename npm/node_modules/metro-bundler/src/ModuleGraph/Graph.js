...
exports.create = function create(resolve, load) {let Graph = (() => {var _ref = _asyncToGenerator(
    function* (entryPoints, platform, options) {var _ref2 =
      ...
      function (_ref3) {let id = _ref3.id,parent = _ref3.parent;return memoizingLoad(resolve(id, parent, platform, options || NO_OPTIONS), loadOptions);},
      onFileLoaded,
...
class Queue {
  ...
  constructor(runTask, accumulate, initial) {this._pending = new Set();this._queue = [];
...
function onFileLoaded(queue, modules, _ref4, _ref5) {let file = _ref4.file,dependencies = _ref4.dependencies;let id = _ref5.id,parent = _ref5.parent,parentDependencyIndex = _ref5.parentDependencyIndex,skip = _ref5.skip;const
  path = file.path;
  ...
  if ((!skip || !skip.has(path)) && !modules.has(path)) {
    ...
    queue.enqueue.apply(queue, _toConsumableArray(dependencies.map((id, i) => ({
      id,
      parent: path,
      parentDependencyIndex: i,
      skip }))));
