...
func (h *CafeService) setAddrs(bindedHttpAddr string, publicAddr string, swarmPorts config.SwarmPorts) {
	publicAddr = strings.TrimRight(publicAddr, "/")
	if publicAddr == "" {
		publicAddr = "http://127.0.0.1"
	}

	// set the http address where peers can reach this cafe
	parts := strings.Split(bindedHttpAddr, ":")
	if len(parts) == 2 {
		h.httpAddr = fmt.Sprintf("%s:%s", publicAddr, parts[1])
