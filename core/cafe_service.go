...
func (h *CafeService) setAddrs(conf *config.Config, swarmPorts config.SwarmPorts) error {
	var ip4, ip6 string

	// if url was not specified, we _must_ aquire a public ip4 address
	...
	if url == "" {
		...
		ip6, _ = ipfs.GetIPv6Addr(h.service.Node())
	}
	log.Infof("cafe url: %s", url)

	swarm := []string{
		fmt.Sprintf("/ip4/%s/tcp/%s", ip4, swarmPorts.TCP),
	}
	if swarmPorts.WS != "" {
		swarm = append(swarm, fmt.Sprintf("/ip4/%s/tcp/%s/ws", ip4, swarmPorts.WS))
	}
	if ip6 != "" {
		swarm = append(swarm, fmt.Sprintf("/ip6/%s/tcp/%s", ip6, swarmPorts.TCP))
		if swarmPorts.WS != "" {
			swarm = append(swarm, fmt.Sprintf("/ip6/%s/tcp/%s/ws", ip6, swarmPorts.WS))
		}
	}
